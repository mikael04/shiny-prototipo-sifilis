---
title: "Relatório de dados de Sífilis"
...
vars <- c("faixa_etaria", "raca", "escolaridade")
...
for(var in vars){
  ...
  df_sifilis_ano_var <- df_sifilis_test |>
    dplyr::select(ano_diag, !!as.name(var), n_casos_sifg) |> 
    dplyr::filter(!is.na(ano_diag)) |>
    dplyr::group_by(ano_diag, !!as.name(var)) |> 
    dplyr::mutate(n = sum(n_casos_sifg)) |> 
    dplyr::arrange(ano_diag, !!as.name(var)) |> 
    dplyr::select(-n_casos_sifg) |>
    dplyr::distinct(ano_diag, !!as.name(var), .keep_all = TRUE)
  
  df_sifilis_ano_var <- df_sifilis_ano_var |> 
    dplyr::mutate(categs_var = ifelse(is.na(!!as.name(var)), "inv", "val")) |>
    dplyr::group_by(ano_diag, categs_var) |>
...
for(var in vars_av){
  ...
  df_sifilis_categoric_vars |>
    rbind(df_sifilis$n_casos_sifg) |>
    tibble::as_tibble() |>
    # dplyr::group_by(!!as.name(var)) |>
    dplyr::select(!!as.name(var), n_casos_sifg) |>
    ggplot2::ggplot() +
    ggplot2::geom_bar(aes(x = !!as.name(var)), fill = "#ABA2D1") +
    ggplot2::labs(title = paste("Distribuição de", !!as.name(var)),
         x = !!as.name(var),
         y = "Número de casos") +
...
df_sifilis_uf |> 
  ...
  gt::gt() |> 
  gt::cols_label(uf_nome = "UF", n_casos_sifg = "Nº de casos", n_nascidos = "Nº de nascidos", taxa_sifilis = "Taxa de sífilis (1000*n_casos/n_nascidos)")
...
df_sifilis_anomalia |> 
  ...
  gt::gt() |> 
  gt::cols_label(ano_diag = "Ano", n_nascidos_anomalia = "Nº de nascidos com anomalia", taxa_anomalia = "Taxa de nascidos com anomalia", n_casos_sifg = "Nº de casos de Sífilis", n_nascidos_vivos = "Nº de nascidos vivos")
