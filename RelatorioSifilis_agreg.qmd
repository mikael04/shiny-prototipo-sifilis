---
title: "Relatório de dados de Sífilis (dados agregados)"
author: "Mikael Marin Coletto"
sidebar: false
toc: true
execute:
  warning: false
cache: FALSE
lang: pt
theme: cerulean
reference-location: margin
code-fold: true
code-summary: "Mostrar o código"
code-link: true
format: 
  html:
    page-layout: full
    output-file: "relatorio_html_agreg"
    embed-resources: true
    anchor-sections: true
    smooth-scroll: true
  pdf:
    toc: true
    number-sections: true
    colorlinks: true
    output-file: "relatorio_pdf_agreg"
editor: visual
params:
  run_all_code: "T"
  overwrite_tables: "T"
  use_db: "T"
  debug: "F"
  teste_interno: "F"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(ggplot2)
library(dplyr)
library(gt)
options(scipen=999)
```

# Sífilis gestacional

## Introdução

Este relatório tem como objetivo descrever o banco de dados de dados longitudinal sobre a Sífilis Gestacional e Congênita no Brasil no período de 2007 a 2023, obtidos do Sistema de Informação de Agravos de Notificação (SINAN). Os dados foram agregados segundo período, município, faixa etária, raça/cor e escolaridade e integram variáveis de origem de outras fontes e sistemas de informações como Sistema de Informações de Nascidos Vivos (SINASC), Cadastro Nacional de Estabelecimentos de Saúde (CNES), Sistema de Informações Hospitalares (SIH), Sistema de Informação de Mortalidade (SIM), Programa Bolsa Família, Cobertura de Estratégia de Saúde da Família e Atenção Primária, indicadores socioeconômicos, entre outros.

## Dados

Os dados foram obtidos do banco de dados do SINAN e estão disponíveis no arquivo `df_sifilis_mun_ano_fx_et_racacor_esc.RDS`. O arquivo contém as seguintes variáveis:

-   Município de residência
-   Ano do diagnóstico
-   Mês do diagnóstico
-   Faixa etária
-   Raça/cor
-   Escolaridade
-   Número de casos de Sífilis

```{r lendo-dados}
sif_13m <- T
if(sif_13m){
  df_sifilis <- readRDS("data-raw/df_sifilis_all.RDS") |>
    janitor::clean_names() |> 
    dplyr::rename(esc = escolaridade, nasc_c_anom = nascidos_com_anomalia, 
                  nasc_viv = nascidos_vivos, raca = cs_raca) |> 
    dplyr::mutate(ano_mes = paste0(ano, "-", mes))
} else {
  df_sifilis <- readRDS("data-raw/df_sifilis_mun_ano_fx_et_racacor_esc.RDS") |>
  janitor::clean_names()
}
head(df_sifilis) |> 
  gt::gt()

```

## Análise Exploratória

### Completude de variáveis (Geral)

```{r analise-completude}
variaveis <- colnames(df_sifilis)

library(dplyr)

## Definindo como NA as variáveis que possuem valores inválidos
df_sifilis_test <- df_sifilis %>%
  dplyr::mutate(across(where(is.character), ~ ifelse(.x %in% c("****", "Em branco/Inválido"), NA, .x)))

df_completude <- data.frame(variavel = character(), completude = numeric())
for(i in variaveis){
  completude = round(100*(sum(!is.na(df_sifilis_test[[i]]))/nrow(df_sifilis_test)), 5)
  df_completude <- rbind(df_completude, data.frame(variavel = i, completude = completude))
}

df_completude |> 
  gt::gt() |> 
  gt::cols_label(variavel = "Variável", completude = "Completude")

```

### Completude de variáveis (por ano)

```{r analise-completude-ano}

vars <- c("fx_etaria", "raca", "esc")

## Período
years <- 2007:2023

## Criando dataframe 
df_var <- data.frame(var = character())

## Adicionando colunas de anos
for (year in years) {
  df_var[, paste0(year)] <- rep(NA, nrow(df_var))  # Add a column named "Yyyy" with NA values
}

for(var in vars){
  # var <- "faixa_etaria"
  df_sifilis_ano_var <- df_sifilis_test |>
    dplyr::select(ano, !!as.name(var), casos_sifg) |> 
    dplyr::filter(!is.na(ano)) |>
    dplyr::group_by(ano, !!as.name(var)) |> 
    dplyr::mutate(n = sum(casos_sifg)) |> 
    dplyr::arrange(ano, !!as.name(var)) |> 
    dplyr::select(-casos_sifg) |>
    dplyr::distinct(ano, !!as.name(var), .keep_all = TRUE)
  
  df_sifilis_ano_var <- df_sifilis_ano_var |> 
    dplyr::mutate(categs_var = ifelse(is.na(!!as.name(var)), "inv", "val")) |>
    dplyr::group_by(ano, categs_var) |>
    dplyr::summarise(n = sum(n)) |>
    dplyr::ungroup() |> 
    tidyr::pivot_wider(names_from = categs_var, values_from = n)
  
  df_sifilis_ano_var <- df_sifilis_ano_var |>
    dplyr::mutate(perc = paste0(round(val/(val + inv)*100, 2), "%")) |> 
    dplyr::select(ano, perc)
  
  
  df_var_aux <- t(df_sifilis_ano_var) |> 
    tibble::as_tibble(.name_repair = "minimal") |> 
    janitor::row_to_names(row_number = 1) |> 
    dplyr::mutate(var = var) |> 
    dplyr::select(var, everything())
  
  df_var <- rbind(df_var, df_var_aux)
}

df_var |> 
  gt::gt() |> 
  gt::cols_label(var = "Variável")


```

### Variáveis numéricas

```{r analise-numericas}
df_sifilis_numeric_vars <- df_sifilis_test |> 
  dplyr::select(where(is.numeric))

df_numeric <- data.frame(variavel = character(), 
                         min = numeric(), median = numeric(), media = numeric() , max = numeric(),
                         total = numeric())

variaveis <- colnames(df_sifilis_numeric_vars)
for(i in variaveis){
  min = min(df_sifilis_numeric_vars[[i]], na.rm = TRUE)
  median = median(df_sifilis_numeric_vars[[i]], na.rm = TRUE)
  media = round(mean(df_sifilis_numeric_vars[[i]], na.rm = TRUE), 2)
  max = max(df_sifilis_numeric_vars[[i]], na.rm = TRUE)
  sum = sum(df_sifilis_numeric_vars[[i]], na.rm = TRUE)
  df_numeric <- rbind(df_numeric, data.frame(variavel = i, 
                                             min = min, median = median, media = media, 
                                             max = max, total = sum))
}

df_numeric |> 
  gt::gt() |> 
  gt::cols_label(variavel = "Variável", min = "Mínimo", median = "Mediana", media = "Média", max = "Máximo", total = "Total")

```

### Variáveis categóricas

### Tabela geral

```{r analise-categoricas}
df_sifilis_categoric_vars <- df_sifilis |> 
  dplyr::select(where(is.character))

variaveis <- colnames(df_sifilis_categoric_vars)

df_categoric <- data.frame(variavel = character(), 
                           n_distinct = numeric(), 
                           levels = character())

for(i in variaveis){
  n_distinct = n_distinct(df_sifilis_categoric_vars[[i]], na.rm = TRUE)
  levels = paste(sort(unique(df_sifilis_categoric_vars[[i]])), collapse = ", ")
  if(length(unique(df_sifilis_categoric_vars[[i]])) > 20){
    levels = "Muitos valores distintos (> 20)"
  }
  df_categoric <- rbind(df_categoric, data.frame(variavel = i, 
                                                 n_distinct = n_distinct, levels = levels))
  # print(df_categoric)
}

df_categoric |> 
  gt::gt() |> 
  gt::cols_label(variavel = "Variável", n_distinct = "Nº de valores distintos", levels = "Valores distintos")

```

#### Gráficos

```{r graficos-categoricas}
df_categoric_av <- df_categoric |>
  dplyr::filter(n_distinct <= 20)

library(ggtext)
vars_av <- df_categoric_av$variavel
grafs_categ <- list()

i <- 1
for(var in vars_av){  
  # var <- "ano"
  
  grafs_categ[[i]] <- df_sifilis_categoric_vars |>
    cbind(df_sifilis$casos_sifg) |>
    dplyr::rename(casos_sifg = `df_sifilis$casos_sifg`) |>
    tibble::as_tibble() |>
    # dplyr::group_by(!!as.name(var)) |>
    dplyr::select(!!as.name(var), casos_sifg) |>
    ggplot2::ggplot() +
    ggplot2::geom_bar(aes(x = !!as.name(var)), fill = "#ABA2D1") +
    ggplot2::labs(
      title = paste("Distribuição da variável ", var, ""),
      x = var,
      y = "Número de casos") +
    ggplot2::theme_minimal() +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1)) +
    ggplot2::scale_y_continuous(labels = scales::number)
  
  i=i+1
}

# grafs_categ[[1]]
grafs_categ

```

#### Distribuição de número de casos de Sífilis

##### Ano de diagnóstico

```{r distribuicao-ano}

df_sifilis_ano <- df_sifilis |>
  dplyr::group_by(ano) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg)) |>
  dplyr::ungroup()

df_sifilis_ano |>
  ggplot2::ggplot() +
  ggplot2::geom_col(aes(x = ano, y = casos_sifg), fill = "#ABA2D1") +
  ggplot2::labs(title = "Distribuição de casos de Sífilis por ano",
       x = "Ano",
       y = "Número de casos") +
  ggplot2::theme_minimal() +
  ggplot2::scale_y_continuous(labels = scales::number)

```

##### Ano/mês de diagnóstico

```{r distribuicao-ano-mes}

df_sifilis_ano_mes <- df_sifilis |>
  dplyr::filter(ano != "****") |> 
  dplyr::group_by(ano, mes) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg)) |>
  dplyr::ungroup() |> 
  dplyr::mutate(mes_ano = paste0(ano, "-", mes, "-", "01")) |> 
  dplyr::mutate(data = as.Date(mes_ano, format = "%Y-%m-%d"))

df_sifilis_ano_mes |>
  ggplot2::ggplot(aes(x = data, y = casos_sifg)) +
  geom_line(fill = "#ABA2D1") +  # Add fill color for better visualization (optional)
  labs(title = "Casos de Sífilis por Mês e Ano",  # Adjust title as needed
       x = "Mês e Ano",
       y = "Número de Casos") +
  theme_minimal() +
  scale_x_date(date_breaks = "year", date_labels = "%Y")  # Adjust x-axis formatting (optional)
```

##### Nos estados

```{r distribuicao-estados}
df_sifilis_uf <- df_sifilis |>
  dplyr::group_by(uf_resi) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg),
                   n_nascidos = sum(nasc_viv)) |>
  dplyr::ungroup() |> 
  dplyr::mutate(taxa_sifilis = casos_sifg / n_nascidos * 1000)

df_ufs <- data.table::fread("data-raw/ibge-ufs-pop-2022-est.csv") |>
  dplyr::mutate(uf_cod = as.character(uf_cod)) |> 
  dplyr::select(uf_cod, uf_nome)

df_sifilis_uf <- dplyr::left_join(df_sifilis_uf, df_ufs, by = c("uf_resi" = "uf_cod"))

uf_sf <- sf::read_sf(here::here("data-raw/dados-espaciais/uf_sf.shp")) |>
  dplyr::select(cod_stt, geometry)  |>
  dplyr::mutate(cod_stt = as.character(cod_stt))

df_sifilis_uf_sf <- df_sifilis_uf |> 
  dplyr::left_join(uf_sf, by = c("uf_resi" = "cod_stt")) |> 
  sf::st_as_sf()

df_sifilis_uf_sf <- sf::st_transform(df_sifilis_uf_sf, crs = '+proj=longlat +datum=WGS84')
```

###### Gráfico

```{r distribuicao-estados-grafico}
df_sifilis_uf_sf |>
  ggplot2::ggplot() +
  ggplot2::geom_sf(aes(fill = taxa_sifilis)) +
  ggplot2::labs(title = "Taxa de casos de Sífilis por UF",
       fill = "Taxa de sífilis (1000*casos/nascidos)") +
  ggplot2::theme_void() +
  ggplot2::theme(legend.position = "bottom") +
  ggplot2::scale_fill_gradient2(low = "#F2D4F3", high = "#791A78") +
  ggplot2::guides(fill = ggplot2::guide_legend(theme = ggplot2::theme(
    legend.title = ggplot2::element_text(size = 15, face = "bold", colour = "black")
  )))
```

###### Tabela

```{r distribuicao-ano-tabela}

df_sifilis_uf |> 
  dplyr::select(uf_nome, casos_sifg, n_nascidos, taxa_sifilis) |>
  dplyr::mutate(taxa_sifilis = round(taxa_sifilis, 2)) |>
  gt::gt() |> 
  gt::cols_label(uf_nome = "UF", casos_sifg = "Nº de casos", n_nascidos = "Nº de nascidos", taxa_sifilis = "Taxa de sífilis (1000*n_casos/n_nascidos)")

```

```{r distribuicao-categoricas}
df_categoric_av <- df_categoric |>
  dplyr::filter(n_distinct <= 20)

vars_av <- df_categoric_av$variavel
i <- vars_av[1]

## Removendo NAs para contagens de categorias
df_sifilis_categoric_vars <- df_sifilis_categoric_vars |>
  dplyr::mutate(across(where(is.character), ~ ifelse(is.na(.x), "NA", .x))) |> 
  dplyr::mutate(ano = ifelse(ano == "****", "NA", ano))

df_vars_categ <- data.frame(variavel = character(), categorias = character())
for(i in vars_av){
  categ <- unique(df_sifilis_categoric_vars[[i]])
  cat("\n")
  table <- df_sifilis_categoric_vars[[i]] |> 
    dplyr::as_tibble() |>
    dplyr::group_by(value) |>
    dplyr::summarise(n = n()) |>
    dplyr::ungroup() |> 
    dplyr::rename(categoria = value)
  
  total <- sum(table$n)
  
  table <- table |> 
    dplyr::mutate(perc = round(n/total*100, 0)) |> 
    dplyr::mutate(row = paste0("**", categoria, ":** ", "</b>", n, " (", perc, "%)")) |> 
    dplyr::select(row)
  
  row <- paste0(table$row, collapse = "; \n")
  # table |> gt::gt()
    # gt::cols_label(categoria = "Categoria", n = "Nº de casos")
  
  df_vars_categ <- rbind(df_vars_categ, data.frame(variavel = i, categorias = row))
  
  # table_print <- gt::gt(table, rowname_col = "row", groupname_col = "group")
  # print(table_print)
  # cat("\n")
}

df_vars_categ |> 
  gt::gt() |>
  gt::cols_label(variavel = "Variável", categorias = "Categorias: N na categoria (% na categoria)") |> 
  gt::fmt_markdown(columns = everything(), rows = everything(), md_engine = "markdown")

```

#### Nascidos com anomalia

```{r nascidos-anomalia}
df_sifilis_anomalia <- df_sifilis |>
  dplyr::group_by(ano) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg),
                   n_nascidos_anomalia = sum(nasc_c_anom),
                   n_nascidos_vivos = sum(nasc_viv)) |>
  dplyr::ungroup() |> 
  dplyr::mutate(taxa_anomalia = n_nascidos_anomalia / n_nascidos_vivos * 1000)

```

##### Tabela

```{r nascidos-anomalia-tabela}
df_sifilis_anomalia |> 
  dplyr::select(ano, n_nascidos_anomalia, taxa_anomalia, casos_sifg, n_nascidos_vivos) |> 
  gt::gt() |> 
  gt::cols_label(ano = "Ano", n_nascidos_anomalia = "Nº de nascidos com anomalia", taxa_anomalia = "Taxa de nascidos com anomalia", casos_sifg = "Nº de casos de Sífilis", n_nascidos_vivos = "Nº de nascidos vivos")


```

##### Gráfico

```{r, nascidos-anomalia-grafico}
library(ggplot2)
df_sifilis_anomalia |>
  dplyr::filter(ano != "****") |> 
  dplyr::mutate(ano = as.numeric(ano)) |> 
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(x = ano, y = taxa_anomalia), color = "#ABA2D1") +
  ggplot2::labs(title = "Distribuição de taxa de nascidos com anomalia por ano",
       x = "Ano",
       y = "Taxa de nascidos com anomalia*") +
  ggplot2::theme_minimal() +
  ggplot2::scale_y_continuous(labels = scales::number) +
  ggplot2::scale_x_continuous(breaks = seq(2007, 2023, 1))

```

##### Correlação em nível municipal de taxa de sífilis e nascidos com anomalia

```{r correlacao-mun-taxa-anomalia}
df_sifilis_mun <- df_sifilis |>
  dplyr::group_by(id_mn_resi) |>
  dplyr::mutate(casos_sifg = sum(casos_sifg),
                n_nascidos_anomalia = sum(nasc_c_anom),
                n_nascidos_vivos = sum(nasc_viv)) |>
  dplyr::ungroup() |> 
  dplyr::distinct(id_mn_resi, .keep_all = TRUE) |>
  dplyr::mutate(taxa_anomalia = round(n_nascidos_anomalia / n_nascidos_vivos * 1000, 2),
                taxa_sifilis = round(casos_sifg / nasc_viv * 1000, 2))

```

Correlação do número de casos de Sífilis com a taxa de nascidos com anomalia:

```{r correlacao-mun-taxa-anomalia-correlacao}
cor(df_sifilis_mun$casos_sifg, df_sifilis_mun$taxa_anomalia, use = "complete.obs")
```

Correlação do número de casos de Sífilis com IDHM:

```{r correlacao-mun-idhm}

cor(df_sifilis_mun$casos_sifg, df_sifilis_mun$idhm_2010, use = "complete.obs")
```

```{r n-casos-categorias-ano-mes}
df_sifilis_mun_ano_mes <- df_sifilis |>
  dplyr::group_by(id_mn_resi, ano, mes) |>
  dplyr::mutate(casos_sifg = sum(casos_sifg),
                ano_mes = paste0(ano, "-", mes),
                taxa_anomalia = round(nasc_c_anom / nasc_viv * 1000, 2),
                taxa_sifilis = round(casos_sifg / nasc_viv * 1000, 2)) |> 
  dplyr::distinct(id_mn_resi, ano, mes, .keep_all = TRUE) |>
  dplyr::ungroup() |> 
  dplyr::select(id_mn_resi, ano, mes, ano_mes, casos_sifg, nasc_viv, taxa_anomalia, taxa_sifilis, fx_etaria, raca, esc, ibp_quintil, idhm_2010, n_ubs, qtd_familias_pbf) |> 
  dplyr::arrange(id_mn_resi, ano, mes)
```

Correlação do número de casos de Sífilis com IBP (agregado por município, ano e mês):

```{r correlacao-mun-ibp}

cor(df_sifilis_mun_ano_mes$casos_sifg, df_sifilis_mun_ano_mes$ibp_quintil, use = "complete.obs")
```

Matriz de correlação de número de casos de sífilis e taxa de anomalia, com IBP, IDHM, quantidade de famílias do PBF e número de UBS:

```{r correlacao-mun-matrix}

corrplot::corrplot(cor(df_sifilis_mun_ano_mes |>
                       dplyr::select(casos_sifg, taxa_sifilis, taxa_anomalia, ibp_quintil, idhm_2010, qtd_familias_pbf, n_ubs), use = "complete.obs"), 
                  method = "number", type = "upper", tl.col = "black", tl.cex = 0.8)
```

## Análise bivariada

### Categóricas (agregadas por município)

#### Variáveis e suas categorias por ano

```{r n-casos-categorias-ano}

df_categoric_av <- df_categoric |>
  dplyr::filter(n_distinct <= 20)

vars <- df_categoric_av$variavel

vars <- vars[-c(2)]
grafs_biv <- list()
i <- 1
for(var in vars){
  # var <- "fx_etaria"
  df_sifilis_biv_ano <- df_sifilis |>
    dplyr::group_by(ano, !!as.name(var)) |>
    dplyr::summarise(casos_sifg = sum(casos_sifg)) |>
    dplyr::ungroup()
  
  grafs_biv[[i]] <- df_sifilis_biv_ano |> 
    ggplot2::ggplot() +
    ggplot2::geom_col(aes(x = ano, y = casos_sifg, fill = !!as.name(var))) +
    ggplot2::labs(title = paste("Distribuição de casos de Sífilis por ano e", var),
         x = paste0("Categorias de ", var),
         y = "Número de casos") +
    ggplot2::theme_minimal() +
    ggplot2::scale_y_continuous(labels = scales::number) + 
    ggplot2::scale_x_discrete(breaks = seq(2006, 2024, 2))
  
  i <- i+1
}

grafs_biv

# df_sifilis_biv_ano |> 
#   ggplot2::ggplot() +
#   ggplot2::geom_col(aes(x = ano, y = casos_sifg, fill = !!as.name(var))) +
#   ggplot2::labs(title = paste("Distribuição de casos de Sífilis por ano e", var),
#        x = paste0("Categorias de ", var),
#        y = "Número de casos") +
#   ggplot2::theme_minimal() +
#   ggplot2::scale_y_continuous(labels = scales::number) + 
#   ggplot2::scale_x_discrete(breaks = seq(2006, 2024, 2))


```

### Taxas de sífilis e outras variáveis socioeconômicas

```{r taxas-sifilis-socioeconomicas}
vars_socio <- c("ibp_quintil", "idhm_2010", "n_ubs", "qtd_familias_pbf")
vars_others <- c("id_mn_resi", "nome_mn_resi", "casos_sifg", "nasc_viv")

vars_all <- c(vars_others, vars_socio)

# var <- vars_socio[2]

grafs_disp <- list()
i <- 1

for(var in vars_socio){
  graf <- df_sifilis_mun |> 
    ggplot2:::ggplot() +
    # ggplot2::geom_point(aes(x = !!as.name(var), y = taxa_sifilis)) + 
    ggplot2::geom_jitter(aes(x = !!as.name(var), y = taxa_sifilis),
                         width = 0.25) +
    ggplot2::labs(title = paste("Dispersão da Taxa de Sífilis por", var),
         x = var,
         y = "Taxa de Sífilis (1000*casos/nascidos)") +
    ggplot2::theme_minimal() +
    ggplot2::scale_y_continuous(labels = scales::number)
  
  grafs_disp[[i]] <- graf
  i <- i+1
}

grafs_disp

```

#### Dispersão das variáveis socioeconômicas em relação à taxa de sífilis

```{r, grafs-disp-vars-socio}
grafs_disp
```

#### Boxplot das variáveis socioeconômicas em relação à taxa de sífilis

```{r, grafs-boxplot-vars-socio}
var <- vars_socio[1]
df_sifilis_mun |> 
  ggplot2:::ggplot() +
  # ggplot2::geom_point(aes(x = !!as.name(var), y = taxa_sifilis)) + 
  ggplot2::geom_boxplot(aes(x = !!as.name(var), y = taxa_sifilis, group= !!as.name(var))) +
  ggplot2::labs(title = paste("Boxplot da Taxa de Sífilis por", var),
       x = var,
       y = "Taxa de Sífilis (1000*casos/nascidos)") +
  ggplot2::theme_minimal() +
  ggplot2::scale_y_continuous(labels = scales::number)

```

### Taxa de sífilis, variáveis socioeconômicas e pessoais

#### Faixa etária por IBP quintil, IDH, quantidade de UBS e quantidade de famílias do PBF

```{r, faixa-etaria-ibp-idh-ubs-pbf}
# df_sifilis_fx_etaria <- df_sifilis_mun |> 
#   dplyr::group_by(fx_etaria) |>
#   dplyr::mutate(casos_sifg = sum(casos_sifg), nasc_viv = sum(nasc_viv),
#                 taxa_sifilis = round(casos_sifg / nasc_viv * 1000, 2)) |>
#   dplyr::distinct(fx_etaria, .keep_all = TRUE) |>
#   dplyr::ungroup()

df_sifilis_mun_fx_etaria <- df_sifilis_mun |> 
  dplyr::group_by(fx_etaria, ibp_quintil) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg), nasc_viv = sum(nasc_viv),
                   taxa_sifilis = round(casos_sifg / nasc_viv * 1000, 2)) |>
  dplyr::ungroup() |> 
  dplyr::select(fx_etaria, ibp_quintil, taxa_sifilis)

df_sifilis_mun_fx_etaria_wide <- df_sifilis_mun_fx_etaria |> 
  tidyr::pivot_wider(names_from = ibp_quintil, values_from = taxa_sifilis) |> 
  dplyr::select(-`NA`) |> 
  dplyr::filter(fx_etaria != "60+")

```

##### Tabela

```{r, faixa-etaria-ibp-tabela}

df_sifilis_mun_fx_etaria_wide |> 
  gt::gt() |> 
  gt::cols_label(fx_etaria = "Faixa etária", `1` = "IBP 1º Q", `2` = "IBP 2º Q", `3` = "IBP 3º Q", `4` = "IBP 4º Q", `5` = "IBP 5º Q")

```

##### Gráfico

```{r, faixa-etaria-ibp-grafico}

df_sifilis_mun_fx_etaria |> 
  ggplot2::ggplot(aes(x = ibp_quintil, y = taxa_sifilis)) +
  # ggplot2::geom_point() +
  # ggplot2::geom_bar(aes(x = fx_etaria,  = `1`, fill = "1º Q"), stat = "identity") +
  ggplot2::geom_line(aes(colour = fx_etaria)) +
  ggplot2::labs(title = "Taxa de Sífilis por faixa etária e IBP",
       x = "IBP Quintil",
       y = "Taxa de Sífilis (1000*casos/nascidos)") +
  ggplot2::theme_minimal() +
  ggplot2::scale_y_continuous(labels = scales::number) +
  ggplot2::scale_color_manual(
    name = "Faixa etária",
    values = c("#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#00FFFF", "#FF00FF", "gray"))

```

#### Faixa etária por IDH

```{r, faixa-etaria-idh}

df_sifilis_mun_fx_etaria_idh <- df_sifilis_mun |> 
  dplyr::mutate(idhm_2010_cat = cut(idhm_2010, breaks = c(0, 0.55, 0.7, 0.8, 1), 
                                   labels = c("Baixo", "Medio", "Alto", "Muito Alto")))

df_sifilis_mun_fx_etaria_idh <- df_sifilis_mun_fx_etaria_idh |> 
  dplyr::group_by(fx_etaria, idhm_2010_cat) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg), nasc_viv = sum(nasc_viv),
                   taxa_sifilis = round(casos_sifg / nasc_viv * 1000, 2)) |>
  dplyr::ungroup() |> 
  dplyr::select(fx_etaria, idhm_2010_cat, taxa_sifilis) |> 
  tidyr::pivot_wider(names_from = idhm_2010_cat, values_from = taxa_sifilis) |> 
  dplyr::select(-`NA`) |> 
  dplyr::filter(fx_etaria != "60+")

df_sifilis_mun_fx_etaria_idh |> 
  gt::gt() |> 
  gt::cols_label(fx_etaria = "Faixa etária", Baixo = "IDH Baixo", Medio = "IDH Médio", Alto = "IDH Alto", `Muito Alto` = "IDH Muito Alto")

```

### Categóricas (agregadas por município, ano, mês)

#### Taxa de sífilis por faixa etária e bolsa família

```{r, taxa-sifilis-faixa-etaria-bolsa-familia}

df_sifilis_mun_ano_mes_ <- df_sifilis_mun_ano_mes |> 
  dplyr::filter(id_mn_resi != "110000") |> 
  dplyr::select(id_mn_resi, ano_mes, fx_etaria, qtd_familias_pbf, casos_sifg, nasc_viv)

df_sifilis_mun_ano_mes_ <- df_sifilis_mun_ano_mes_ |> 
  dplyr::mutate(
      familias_pbf_cat = case_when(
      qtd_familias_pbf == 0 ~ 0,
      qtd_familias_pbf > 0 & qtd_familias_pbf <= 100000 ~ 1,
      qtd_familias_pbf > 100000 & qtd_familias_pbf <= 200000 ~ 2,
      qtd_familias_pbf > 200000 & qtd_familias_pbf <= 300000 ~ 3,
      qtd_familias_pbf > 300000 & qtd_familias_pbf <= 400000 ~ 4,
      qtd_familias_pbf > 400000 & qtd_familias_pbf <= 500000 ~ 5,
      qtd_familias_pbf > 500000 ~ 6
    )
  )

df_sifilis_mun_ano_mes_ <- df_sifilis_mun_ano_mes_ |> 
  dplyr::mutate(ano_mes = as.Date(paste0(ano_mes, "-01"), format = "%Y-%m-%d")) |> 
  dplyr::group_by(fx_etaria, familias_pbf_cat) |>
  dplyr::summarise(casos_sifg = sum(casos_sifg), nasc_viv = sum(nasc_viv),
                   taxa_sifilis = round(casos_sifg / nasc_viv * 1000, 2)) |>
  dplyr::ungroup()

df_sifilis_mun_ano_mes_ |>
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(x = familias_pbf_cat, y = taxa_sifilis, colour = fx_etaria)) +
  ggplot2::labs(title = "Taxa de Sífilis por faixa etária e quantidade de famílias do PBF",
       x = "Famílias do PBF",
       y = "Taxa de sífilis",
       colour = "Faixa Etária") +
  ggplot2::theme_minimal() +
  # ggplot2::scale_x_continuous(labels = scales::number) +
  ggplot2::scale_y_continuous(labels = scales::number) + 
  ggplot2::scale_x_continuous(breaks = seq(0, 6, 1))

```

## 
