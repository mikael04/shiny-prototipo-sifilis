...
+datum=WGS84')
  ...
  graf_ano_mes <- tabela_ano_mes |>
    ggplot2::ggplot() +
    ggplot2::geom_line(aes(x = ano_mes, y = n_casos_sifg), color = "#ABA2D1") +
...
+datum=WGS84')
  ...
  graf_racacor_ano <- tabela_racacor_ano |>
    dplyr::filter(raca == "Parda") |>
    ggplot2::ggplot() +
    ggplot2::geom_col(aes(x = ano_diag, y = n_casos_sifg, fill = raca)) +
  ...
  graf_racacor_mes <- tabela_racacor_mes |>
    ggplot2::ggplot() +
    ggplot2::geom_line(aes(x = ano_mes, y = n_casos_sifg, color = raca)) +
